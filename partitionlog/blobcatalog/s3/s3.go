...
func NewBackend(client *awss3.Client, bucket string) (*Backend, error) {
	if client == nil {
		return nil, fmt.Errorf("blobcatalog/s3: nil client")
	}
	if bucket == "" {
		return nil, fmt.Errorf("blobcatalog/s3: empty bucket")
	}
...
func (b *Backend) List(ctx context.Context, opts blobcatalog.ListOptions) (blobcatalog.ObjectPage, error) {
	...
	objects := make([]blobcatalog.ObjectInfo, 0, len(out.Contents))
	for _, item := range out.Contents {
		...
		if item.Size != nil && *item.Size > int64(math.MaxInt) {
			return blobcatalog.ObjectPage{}, fmt.Errorf("%w: object %s size=%d exceeds int", blobcatalog.ErrCorruptCatalog, key, *item.Size)
		}
		size := 0
		if item.Size != nil {
			size = int(*item.Size)
...
func (b *Backend) putObject(ctx context.Context, key string, body []byte, ifNoneMatch *string, ifMatch *string) (blobcatalog.Object, error) {
	out, err := b.client.PutObject(ctx, &awss3.PutObjectInput{
		Bucket:        aws.String(b.bucket),
		Key:           aws.String(key),
		Body:          bytes.NewReader(body),
		ContentLength: aws.Int64(int64(len(body))),
		ContentType:   aws.String(blobcatalog.ObjectContentType),
