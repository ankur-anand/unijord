...
func NewStore(client *awss3.Client, bucket string) (*Store, error) {
	if client == nil {
		return nil, fmt.Errorf("blobsource/s3: nil client")
	}
	if bucket == "" {
		return nil, fmt.Errorf("blobsource/s3: empty bucket")
	}
...
func (s *Store) ReadAt(ctx context.Context, key string, off uint64, n uint64) ([]byte, error) {
	if key == "" {
		return nil, fmt.Errorf("blobsource/s3: empty key")
	}
	if n == 0 {
		return []byte{}, nil
	}
...
func rangeEnd(off uint64, n uint64) (uint64, error) {
	if off > ^uint64(0)-n+1 {
		return 0, fmt.Errorf("blobsource/s3: range overflow offset=%d length=%d", off, n)
	}
	return off + n - 1, nil
}
