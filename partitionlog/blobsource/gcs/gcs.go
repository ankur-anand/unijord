...
func NewStore(client *storage.Client, bucket string) (*Store, error) {
	if client == nil {
		return nil, fmt.Errorf("blobsource/gcs: nil client")
	}
	if bucket == "" {
		return nil, fmt.Errorf("blobsource/gcs: empty bucket")
	}
...
func (s *Store) ReadAt(ctx context.Context, key string, off uint64, n uint64) ([]byte, error) {
	if key == "" {
		return nil, fmt.Errorf("blobsource/gcs: empty key")
	}
	if n == 0 {
		return []byte{}, nil
	}
	if off > math.MaxInt64 || n > math.MaxInt64 {
		return nil, fmt.Errorf("blobsource/gcs: range overflows int64 offset=%d length=%d", off, n)
	}
