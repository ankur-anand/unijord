...
func NewStore(container *container.Client) (*Store, error) {
	if container == nil {
		return nil, fmt.Errorf("blobsource/azure: nil container client")
	}
...
func (s *Store) ReadAt(ctx context.Context, key string, off uint64, n uint64) ([]byte, error) {
	if key == "" {
		return nil, fmt.Errorf("blobsource/azure: empty key")
	}
	if n == 0 {
		return []byte{}, nil
	}
	if off > math.MaxInt64 || n > math.MaxInt64 {
		return nil, fmt.Errorf("blobsource/azure: range overflows int64 offset=%d length=%d", off, n)
	}
